...
def plot_reconstruction_errors(file_errors, threshold):
    ...
    # ファイル名に日本語が含まれている場合は赤、そうでなければ青
    bar_colors = ['red' if contains_japanese(name) else 'blue' for name in file_names]

    plt.figure(figsize=(12, 6))
    plt.bar(range(len(errors)), errors, color=bar_colors, label='Reconstruction Error')
...
def test_model():
    ...
    model.load_state_dict(torch.load("best_autoencoder-5-1.pth", map_location=device))
    model.eval()
    ...
    # 閾値は平均+2σで計算（ここでは例として固定値も設定しています）
    threshold = mean_err + 2 * std_err
    threshold = 0.002  # 必要に応じて調整してください
